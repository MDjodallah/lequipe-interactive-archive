...
export default function CollectionPage({
    ...
    return (
        <div className={styles.screen}>
            {/* Préchargement invisible des images de la collection pour fluidifier les animations */}
            <div style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', visibility: 'hidden', zIndex: -1 }}>
                {RANK_TIERS.map(t => <img key={`preload-tier-${t.id}`} src={withBasePath(t.image)} alt="preload" />)}
                <img src={CARD_BACK} alt="preload" />
                <img src={withBasePath('/couverture/entrée-paroles-de-légendes.png')} alt="preload" />
                {interviews.map(iv => {
                    const slug = iv.slug;
                    const front = getCardFront(slug);
                    const block = getCardBlock(slug);
                    return (
                        <span key={`preload-${slug}`}>
                            {front && <img src={front} alt="preload" />}
                            {block && <img src={block} alt="preload" />}
                        </span>
                    );
                })}
            </div>
