...
export default function CoverPage() {
    ...
    return (
        <div className={styles.screen}>
            {/* Préchargement invisible des images pour garantir l'ordre de l'animation */}
            <div style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', visibility: 'hidden', zIndex: -1 }}>
                {INTRO_PLAYERS.map(p => (
                    <img key={`preload-${p.id}`} src={withBasePath(p.image)} alt="preload" />
                ))}
                <img src={withBasePath('/home/images/coupe.png')} alt="preload" />
            </div>
